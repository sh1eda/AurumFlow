# XAUUSD Explainable Signal System Plan
...
- Valid BUY/SELL requires all hard gates: closed candle data, non-neutral HTF draw, confirmed sweep, body-close MSS in signal direction, retraceable FVG entry zone, structural stop, opposing-liquidity target, minimum RR `2.0`, operating-mode allowance, and confidence `>= 0.70`.
...
- Implement backtesting as a closed-bar event simulator with next-bar or conservative limit-zone fills, spread/slippage/commission costs applied to all exits, no same-bar hindsight, explicit missed-entry handling, and full signal/rejection logs.
...
- Signal tests for valid long, valid short, ML conflict, RR too low, neutral HTF bias, missing sweep, missing MSS, missing entry zone, and model unavailable.
- Backtest tests for next-bar execution, FVG zone fill, stop-before-target conservative ordering, costs, no open-order real-money side effects, and reproducible walk-forward splits.
